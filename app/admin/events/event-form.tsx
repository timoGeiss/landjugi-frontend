...
export function EventForm({ event }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { ...form, end_date: form.end_date || null, start_time: form.start_time || null, end_time: form.end_time || null, image_url: form.image_url || null, pdf_url: form.pdf_url || null, created_by: user?.id }
    const { error } = event
  ...
  return (
    <>
      <Button onClick={() => setOpen(true)} size="sm">
        <Plus className="h-4 w-4" /> {event ? 'Bearbeiten' : 'Neuer Anlass'}
      </Button>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Input value={form.image_url} onChange={e => setForm(f => ({ ...f, image_url: e.target.value }))} placeholder="URL oder hochladen" className="flex-1" />
              ...
              <div className="space-y-1.5">
                <Label>PDF (Flyer)</Label>
                <div className="flex gap-2">
                  <Input value={form.pdf_url} onChange={e => setForm(f => ({ ...f, pdf_url: e.target.value }))} placeholder="URL oder hochladen" className="flex-1" />
                  <label className="cursor-pointer">
